...
df1=read_csv('https://raw.githubusercontent.com/AlissonRP/Medical-Cost-Personal/main/airports.csv')
df2=read_csv('https://raw.githubusercontent.com/AlissonRP/Medical-Cost-Personal/main/hdi.csv')
df3=read_csv('https://raw.githubusercontent.com/AlissonRP/Medical-Cost-Personal/main/states.csv')
df4 = read_csv('https://raw.githubusercontent.com/AlissonRP/Medical-Cost-Personal/main/icu-beds.csv')
df1=
  left_join(df1,df2) %>% 
   left_join(df3) %>% 
     left_join(df4)
...
\section{Introdução}
A proposta do respectivo trabalho é predizer o produto interno bruto (GDP) de 26 estados do Brasil, para isso utiliza-se de um modelo de regressão linear, random forest e Knn
...
```
Perceba a relação entre **Pobreza** e e o **PIB** pelo seguinte gráfico de dispersão:
```{r}
df1 %>% 
  d(Poverty,GDP,0.07)+
...
ft2 <- lm(formula =GDP ~  + `HDI Education 2017` + 
      + Area + `Demographic Density` + 
     +Poverty, data = df1)
residuo = rstudent(ft2)
lmtest::resettest(ft2)$p.value
lmtest::bptest(ft2, studentize = TRUE)$p.value
tseries::jarque.bera.test(residuo)$p.value
lmtest::bptest(ft2, studentize = TRUE)
car::vif(ft2)
summary(ft2)
```
