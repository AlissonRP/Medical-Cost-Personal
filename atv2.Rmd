...
#Se quiser achar um jeito pra completa pela média sinta-se a vontade
df <- df %>%
  select(-Country, -Region, -Climate) %>%
  drop_na()
colnames(df) <- c('Pop', 'Area', 'Densidade', 'Fronteira_Mar', 'Migracao', 'Taxa_Mortalidade_Infantil', 'PIB', 'Alfabetizao', 'Taxa_Telefone', 'Aravel%', 'Cultivo%', 'Outros%', 'Taxa_Natalidade', 'Taxa_Mortalidade', 'Agricultura', 'Industria', 'Servico')
df <- df %>%
  mutate(Densidade = as.numeric(Densidade))
...
```
```{r}
fit <- lm(Alfabetizao~.,data=df) 
summary(fit)
stats::step(fit)
fit2 <- lm(formula = Alfabetizao ~ Migracao + Taxa_Mortalidade_Infantil + 
    Taxa_Telefone + `Aravel%` + `Cultivo%` + Taxa_Natalidade + 
    Taxa_Mortalidade + Agricultura + Industria + Servico, data = df)
jarque.bera.test(resid(fit2))
#Melhor foi com Alfabetizao como dependente
```
