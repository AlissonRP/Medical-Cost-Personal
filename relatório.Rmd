...
\section{Introdução}
A proposta do respectivo trabalho é predizer o produto interno bruto per capita (PIB per capita) de 26 estados do Brasil no ano de 2019, os dados foram extraidos de planinhas disponíveis no site do Instituto Brasileiro de Geografia e Estatística (IBGE) para isso utiliza-se  como variáveis explicativas (covariáveis):
...
df %>% 
  ...
  labs(title="PIB por estado",y="Estado") + scale_x_continuous(
    expand = c(0.01, 0),limits = c(0, 55000))
```
...
df %>% 
  ...
  tbl("PIB por Região")
```
Note que, em média a região Sul possui maior PIB médio e com menor variância tendo em vista também o gráfico \@ref(fig:graphstates), e a região Sudeste possui a maior variação no PIB dado o fato que possui duas regiões com PIBs discrepantes em relação aos demais.  
...
Amedia dos erros é zero, Homoscedasticidade dos erros, Não-autocorrelação, Ausência de Multicolinearidade e Normalidade dos erros.
...
E por último é necessário testar a existência de autocorrelação, usaremos o Teste de Durbin-Watson, que tem por hipótese, que existe não existe correlação, após aplicação do teste obtém-se um p-valor de `r lmtest::dwtest(ft2)$p.value` i.e, não existem indicío contra a hipótese de autocorrelação.
...
tidy(fit2) %>% 
  ...
  kable_classic(latex_options = "HOLD_position") 
```
Que informa que o intercepto  não são significativos a 1%, tem-se um p-valor << 0.001 do teste F e $R^2$ dado por `r round(glance(ft2)[1],3)` que informa que aproximadamente `r round(glance(ft2)[1],3)*100`% da variação do PIB per capita dos estados é explicada pelas covariáveis propostas.
...
\subsection{Apêndice A: Modelagem}
Neste apêndice serão analisados outros modelos preditivos junto com o regressão linear, como os modelos de florestas aleatórias (rf) e os k-vizinhos mais próximos (knn), que pode ser acessado clicando [aqui](https://www.google.com/search?q=modelos+preditivos&rlz=1C1GCEA_pt-BRBR939BR939&oq=modelos+preditivos&aqs=chrome..69i57.2248j0j7&sourceid=chrome&ie=UTF-8).


\subsection{Apêndice B: Análise Descritiva, influência e pressupostos Extra}

Neste apêndice serão feitas análises extras que são interessante para a compreensão da variável desfecho, mas não necessariamente fundamentais, que pode ser acessado clicando [aqui](https://github.com/AlissonRP).

\subsection{Apêndice C: Código Completo}
