...
Nessa seção serão ajustados os modelos de regressão linear, floresta aleatória(rf) e e k-vizinhos mais próximos (knn) [^1], usando as covariáveis já citadas.  
...
```{r}
df_work=
   workflow_set(list(SDF=df_rec1,CDF=df_rec2),list(reg=df_reg,rf=df_rf,knn=df_knn),cross = T)
```
...
df_tuner %>% 
   extract_workflow_set_result(id='SDF_knn') %>% 
...
```
Perceba que a regressão linear possui um rmse menor (`r rank_results(df_tuner,rank_metric = "rmse") %>% .[1,4]`) e a regressão linear possui o maior erro padrão (`r rank_results(df_tuner,rank_metric = "rmse") %>% .[2,5]`)
